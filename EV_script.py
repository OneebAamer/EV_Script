...
class Vehicle:
    def __init__(self, name, price, distpcharge, chargeTime, max_load):
        self.name = name
        self.price = price
    ...
    for i in range(0,len(vehicleArray)):
            for j in range(i,len(vehicleArray)):
                if i == j:
                    ...
                    score = distpcharge + load - price - chargeTime
                    if score > 0:
                        scores[i] += 1
                    elif score < 0:
                        scores[j] += 1
...
def chooseInput():
    ...
    for i in range(0,num_of_cars):
        ...
        add_vehicle = Vehicle(name,price,distpcharge,chargeTime,max_load)
    ...
    with open('data.csv','r', newline='') as csvfile:
        ...
        for row in reader:
            if row[0] == 'END':
                ...
                load = float(row[4])
                add_vehicle = Vehicle(name,price,distpcharge,chargeTime,load)
                vehicle_list.append(add_vehicle)
    compare(vehicle_list)
    winner(scores,cars)
    graph(vehicle_list)
